...
contains
    subroutine test_sanity()
        ...
        call sgersvd1(-1, -1, fun, fun, -1, foo, 0, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -1)

        call sgersvd1(0, -1, fun, fun, -1, foo, 0, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -2)

        call sgersvd1(0, 0, fun, fun, -1, foo, 0, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -5)

        call sgersvd1(0, 0, fun, fun, 1, foo, 0, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -5)

        call sgersvd1(0, 0, fun, fun, 0, foo, 0, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -7)

        call sgersvd1(0, 0, fun, fun, 0, foo, 1, -1, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -8)

        call sgersvd1(0, 0, fun, fun, 0, foo, 1, 0, foo, foo, 0, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -11)

        call sgersvd1(0, 0, fun, fun, 0, foo, 1, 0, foo, foo, 1, foo, 0, foo, -1, ifoo, -1, info)
        ASSERT(info == -13)
    ...
    subroutine test_quick()
        ...
        call sgersvd1(0, 1, fun, fun, 1, foo, 1, 0, foo, foo, 1, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)

        call sgersvd1(1, 0, fun, fun, 0, foo, 1, 0, foo, foo, 1, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)

        call sgersvd1(1, 1, fun, fun, 0, foo, 1, 0, foo, foo, 1, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)
    ...
    !-- niter = 0
        ...
        call sgersvd1(m, n, fun, fun, kc, foo, ldcsk, niter, foo, foo, ldu, foo, ldvt, foo, -1, ifoo, -1, info)
        ASSERT(info == 0)
        lwork = int(foo(1))
        liwork = ifoo(1)
        ...
        call rng%snormal(n*kc, gauss, ZERO, ONE, info)
        call sgemm('n', 'n', m, kc, n, ONE, A, lda, gauss, n, ZERO, csk, ldcsk)

        call sgersvd1(m, n, B2AB, B2BA, kc, csk, ldcsk, niter, S, U, ldu, VT, ldvt, work, lwork, iwork, liwork, info)
    ...
    !-- niter = 2
        ...
        call sdgmm('r', m, kc, U, ldu, S, 1, info)
        call sgemm('n', 'n', m, n, r, -ONE, U, ldu, VT, ldvt, ONE, A, lda)
        err_rsvd1 = sgenrmf(m, n, A, lda, info)
