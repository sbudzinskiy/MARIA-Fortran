...
contains
    subroutine test_sanity()
        ...
        vol = sgevolume(-1, -1, -1, foo, 0, foo, 0, ifoo, 0, info)
        ASSERT(info == -1)
        ...
        vol = sgevolume(1, 1, 2, foo, 0, foo, 0, ifoo, 0, info)
        ASSERT(info == -3)
    ...
    subroutine test_quick()
        ...
        vol = sgevolume(0, 1, 0, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)
        ASSERT(safe_eq(vol, ZERO))

        vol = sgevolume(1, 0, 0, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)
        ASSERT(safe_eq(vol, ZERO))

        vol = sgevolume(1, 1, 0, foo, 1, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)
        ASSERT(safe_eq(vol, ZERO))
    ...
    !-- m = n = r
        ...
        lwork = int(foo(1))
        vol = sgevolume(m, n, r, foo, lda, foo, -1, ifoo, -1, info)
        ASSERT(info == 0)
        ...
        call rng%suniform(min(m,n), S, 0.5_WP, 1.5_WP, info)
        call slasrt('d', min(m,n), S, info)
        ...
        vol = sgevolume(m, n, r, A, lda, work, lwork, iwork, liwork, info)
        ASSERT(are_close(vol, volprod, info, rtol=1e-5_WP))
