...
contains
    subroutine test_sanity()
        ...
        call dgemaxvol(-1, -1, fun, -1, ifoo, ifoo, -1, ONE-EPS, foo, 0, ifoo, 0, info)
        ASSERT(info == -1)
    ...
    subroutine test_quick()
        ...
        call dgemaxvol(0, 1, fun, 0, ifoo, ifoo, 1, ONE, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)

        call dgemaxvol(1, 0, fun, 0, ifoo, ifoo, 1, ONE, foo, 0, ifoo, 0, info)
        ASSERT(info == 0)
    ...
    !-- check
        ...
        lwork = int(foo(1))
        liwork = ifoo(1)
        call dmatcross(m, n, fun, r, ifoo, r, ifoo, r, foo, ldu, foo, ldvt, foo, -1, ifoo, -1, info)
