...
contains
    subroutine test_sanity(seed)
        ...
        call rng%dnormal(-1, foo, ZERO, -EPS, info)
        ASSERT(info == -2)

        call rng%dnormal(10, foo, ZERO, -EPS, info)
        ASSERT(info == -5)

        call rng%dnormal(10, foo, ZERO, ZERO, info)
        ASSERT(info == -5)
    ...
    subroutine test_quick(seed)
        ...
        call rng%dnormal(0, foo, ZERO, ONE, info)
        ASSERT(info == 0)
    ...
    subroutine test(seed)
        ...
        call rng%dnormal(n, x, mean, std, info)
        ASSERT(info == 0)
