...
contains
    subroutine test_sanity()
        ...
        call sttsvd('x', 1, n, foo, ifoo, cores, foo, 0, ifoo, 0, ierr, maxr=maxr, rtolf=-ONE)
        ASSERT( ierr == -1 )

        call sttsvd('l', 1, n, foo, ifoo, cores, foo, 0, ifoo, 0, ierr, maxr=maxr, rtolf=-ONE)
        ASSERT( ierr == -2 )

        n(1) = -1
        call sttsvd('l', 3, n, foo, ifoo, cores, foo, 0, ifoo, 0, ierr, maxr=maxr, rtolf=-ONE)
        ASSERT( ierr == -3 )
    ...
    subroutine test_quick()
        ...
        n(1) = 0
        call sttsvd('l', 3, n, foo, ifoo, cores, foo, 0, ifoo, 0, ierr, maxr=maxr)
        ASSERT( ierr == 0 )

        n(1) = 2
        maxr(1) = 0
        call sttsvd('l', 3, n, foo, r, cores, foo, 0, ifoo, 0, ierr, maxr=maxr)
        ASSERT( ierr == 0 )
    ...
    !-- i1 + i2 + ... + id
        ...
        call sttsvd('l', d, n, foo, r, cores, foo, -1, ifoo, -1, ierr, rtolf=rtolf)
        lwork = int(foo(1))
        liwork = ifoo(1)
        ...
        call scopy(numel, A, 1, tmp, 1)

        call sttsvd('l', d, n, tmp, r, cores, work, lwork, iwork, liwork, ierr, rtolf=rtolf, rerrf=rerrf)
        ASSERT( ierr == 0 )
        ASSERT( all(r == 2) )
        ASSERT( safe_leq(rerrf, rtolf) )
        call stt2full(d, n, r, cores, tmp, work, lwork, ierr)
        call saxpy(numel, -ONE, A, 1, tmp, 1)
        nrmA = snrm2(numel, A, 1)
        aerrf = snrm2(numel, tmp, 1)
        ...
        call scopy(numel, A, 1, tmp, 1)
        call sttsvd('r', d, n, tmp, r, cores, work, lwork, iwork, liwork, ierr, maxr=maxr, rerrf=rerrf)
        ASSERT( ierr == 0 )
        ASSERT( all(r <= maxr) )
        call stt2full(d, n, r, cores, tmp, work, lwork, ierr)
        call saxpy(numel, -ONE, A, 1, tmp, 1)
