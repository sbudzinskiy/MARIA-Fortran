...
contains
    subroutine test_sanity()
        ...
        call stt2full(1, n, r, cores, foo, foo, 0, ierr)
        ASSERT( ierr == -1 )

        n(1) = -1
        call stt2full(3, n, r, cores, foo, foo, 0, ierr)
        ASSERT( ierr == -2 )
    ...
    subroutine test_quick()
        ...
        n(1) = 0
        call stt2full(3, n, r, cores, A, foo, 0, ierr)
        ASSERT( ierr == 0 )
        ...
        call stt2full(3, n, r, cores, A, foo, 0, ierr)
        ASSERT( ierr == 0 )
        ASSERT( sall_const(numel, A, 1, ZERO, ierr) )
    ...
    !-- i1 + i2 + ... + id
        r = 2
        numel = product(n)
        call stt2full(d, n, r, cores, foo, foo, -1, ierr)
        lwork = int(foo(1))
        ...
        call stt2full(d, n, r, cores, A, work, lwork, ierr)
        ASSERT( ierr == 0 )
