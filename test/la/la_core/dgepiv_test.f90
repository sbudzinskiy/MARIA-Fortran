...
contains
    subroutine test_sanity()
        ...
        call dgepiv('x', 'x', 0, 0, foo, -1, -1, -1, ifoo, info)
        ASSERT(info == -1)

        call dgepiv('r', 'x', 0, 0, foo, -1, -1, -1, ifoo, info)
        ASSERT(info == -2)

        call dgepiv('r', 'f', 0, 0, foo, -1, -1, -1, ifoo, info)
        ASSERT(info == -3)

        call dgepiv('r', 'f', 1, 0, foo, -1, -1, -1, ifoo, info)
        ASSERT(info == -4)

        call dgepiv('r', 'f', 1, 2, foo, -1, -1, -1, ifoo, info)
        ASSERT(info == -6)

        call dgepiv('r', 'f', 1, 2, foo, 1, 0, -1, ifoo, info)
        ASSERT(info == -7)

        call dgepiv('r', 'f', 1, 2, foo, 1, 2, -1, ifoo, info)
        ASSERT(info == -7)

        call dgepiv('c', 'f', 1, 2, foo, 1, 3, -1, ifoo, info)
        ASSERT(info == -7)

        call dgepiv('r', 'f', 1, 2, foo, 1, 1, 0, ifoo, info)
        ASSERT(info == -8)

        call dgepiv('r', 'f', 1, 2, foo, 1, 1, 2, ifoo, info)
        ASSERT(info == -8)

        call dgepiv('c', 'f', 1, 2, foo, 1, 1, 3, ifoo, info)
        ASSERT(info == -8)
    ...
    !-- what = 'r', dir = 'f' --------------------------------------------------
        ...
        call dlacpy('a', lda, n, A, lda, tmp, ldA)
        call arange(m, order, 1, 1, info)
        ipiv(1) = 4
        ipiv(2) = 5
        ipiv(3) = 3
        ipiv(4) = 3
        call dgepiv(what, dir, m, n, A, ldA, from, to, ipiv, info, order)
        ASSERT(info == 0)
        ASSERT(order(1) == 4)
        ASSERT(order(2) == 5)
        ASSERT(order(3) == 1)
    ...
    !-- what = 'r', dir = 'f' --------------------------------------------------
        ...
        ipiv(1) = 0
        ipiv(2) = m + 1
        call dgepiv(what, dir, m, n, foo, ldA, from, to, ipiv, info)
        ASSERT(info == 1)
        ipiv(1) = 1
        call dgepiv(what, dir, m, n, foo, ldA, from, to, ipiv, info)
        ASSERT(info == 2)
