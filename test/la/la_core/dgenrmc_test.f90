...
contains
    ...
    subroutine test_quick()
        ...
        nrm = dgenrmc(0, 1, foo, 1, info, pos=pos)
        ASSERT(info == 0)
        ASSERT(safe_eq(nrm, ZERO))
        ASSERT(pos(1) == 1)
        ASSERT(pos(2) == 1)

        nrm = dgenrmc(1, 0, foo, 1, info, pos=pos)
        ASSERT(info == 0)
        ASSERT(safe_eq(nrm, ZERO))
        ASSERT(pos(1) == 1)
        ASSERT(pos(2) == 1)
    ...
    !---------------------------------------------------------------------------
        ...
        lda = m + 1
        allocate(A(lda*n))
        call rng%duniform(lda*n, A, -ONE, ONE, info)
        nrm = dgenrmc(m, n, A, lda, info)
        ASSERT(info == 0)
        ...
        A(i + (j-1)*ldA) = 2 * ONE
        A(i + j*ldA) = 2 * ONE
        nrm = dgenrmc(m, n, A, lda, info, pos=pos)
