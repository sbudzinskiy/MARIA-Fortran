...
contains
    subroutine test_sanity()
        ...
        dot = sgedotf('a', -1, -1, foo, 0, foo, 0, info)
        ASSERT(info == -1)

        dot = sgedotf('n', -1, -1, foo, 0, foo, 0, info)
        ASSERT(info == -2)
        ...
        dot = sgedotf('n', 1, 1, foo, 1, foo, 0, info)
        ASSERT(info == -7)
    ...
    subroutine test_quick()
        ...
        dot = sgedotf('n', 0, 1, foo, 1, foo, 1, info)
        ASSERT(info == 0)
        ASSERT(safe_eq(dot, ZERO))

        dot = sgedotf('n', 1, 0, foo, 1, foo, 1, info)
        ASSERT(info == 0)
        ASSERT(safe_eq(dot, ZERO))
    ...
    !-- transA = 'n' -----------------------------------------------------------
        ...
        dot1 = sgedotf(transA, m, n, A, lda, B, ldb, info)
        ASSERT(info == 0)
    ...
    !-- transA = 'n', ldA == ldB == m  -----------------------------------------
        ...
        dot1 = sgedotf(transA, m, n, A, ldA, B, ldB, info)
        dot2 = sgedotf(transA, ldA, n, A, ldA, B, ldB, info)
        ASSERT(are_close(dot1, dot2, info, rtol=1e-5_WP))
    end subroutine test
end program sgedotf_test
