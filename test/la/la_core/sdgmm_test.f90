...
contains
    subroutine test_sanity()
        ...
        call sdgmm('x', -1, -1, foo, 0, foo, 0, info)
        ASSERT(info == -1)

        call sdgmm('l', -1, -1, foo, 0, foo, 0, info)
        ASSERT(info == -2)
    ...
    subroutine test_quick()
        ...
        call sdgmm('l', 0, 1, foo, 1, foo, 0, info)
        ASSERT(info == 0)

        call sdgmm('l', 1, 0, foo, 1, foo, 0, info)
        ASSERT(info == 0)
    ...
    !-- side = 'l', incD > 0 ---------------------------------------------------
        ...
        call rng%snormal(lend, D, ZERO, ONE, info)
        call sdgmm(side, m, n, A, ldA, D, incD, info)
        ASSERT(info == 0)
        do i = 1, m
            call sscal(n, D(1 + (i-1)*incD), tmp(i:), m)
    ...
    !-- side = 'l', incD < 0 ---------------------------------------------------
        ...
        ASSERT(info == 0)
        do i = 1, m
            call sscal(n, D(lend + (i-1)*incD), tmp(i:), m)
            ASSERT(all_close(n, tmp(i:), m, A(i:), ldA, info, rtol=1e-5_WP))
        end do
        deallocate(A, D, tmp)
    ...
    !-- side = 'r', incD > 0 ---------------------------------------------------
        ...
        ASSERT(info == 0)
        do i = 1, n
            call sscal(m, D(1 + (i-1)*incD), tmp(1 + (i-1)*m:), 1)
    ...
    !-- side = 'r', incD < 0 ---------------------------------------------------
        ...
        ASSERT(info == 0)
        do i = 1, n
            call sscal(m, D(lenD + (i-1)*incD), tmp(1 + (i-1)*m:), 1)
            ASSERT(all_close(m, tmp(1 + (i-1)*m:), 1, A(1 + (i-1)*ldA:), 1, info, rtol=1e-5_WP))
        end do
        deallocate(A, D, tmp)
