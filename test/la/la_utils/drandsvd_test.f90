...
contains
    subroutine test_sanity(seed)
        ...
        call drandsvd(rng, -1, -1, foo, 0, foo, foo, -1, info)
        ASSERT(info == -2)

        call drandsvd(rng, 2, -1, foo, 0, foo, foo, -1, info)
        ASSERT(info == -3)

        call drandsvd(rng, 2, 1, foo, 0, foo, foo, 0, info)
        ASSERT(info == -5)

        call drandsvd(rng, 2, 1, foo, 1, foo, foo, 0, info)
        ASSERT(info == -5)

        call drandsvd(rng, 2, 1, foo, 2, foo, foo, 0, info)
        ASSERT(info == -8)
    ...
    subroutine test_quick(seed)
        ...
        call drandsvd(rng, 1, 0, foo, 1, foo, foo, 0, info)
        ASSERT(info == 0)

        call drandsvd(rng, 0, 1, foo, 1, foo, foo, 0, info)
        ASSERT(info == 0)
    ...
    subroutine test(seed)
        ...
        lwork = int(foo(1))
        call dgesdd_q('n', m, n, foo, ldA, foo, foo, 1, foo, 1, foo, -1, ifoo, -1, info)
        ...
        ASSERT(info == 0)
        call dgesdd_q('n', m, n, A, ldA, SS, foo, 1, foo, 1, work, lwork, iwork, liwork, info)
        write(*, '(10es12.5)') S
        write(*, '(10es12.5)') SS
